...
VALUES
  ('pumpkin candle', 'Jolly Man', '10', FALSE),
  ('nutcracker', 'Elf', '18', TRUE),
  ('Snowballs', 'Snowman', '3', TRUE),
  ('Bicycle', 'St, Nick', '50', FALSE),
  ('Doll', 'Mystery Man', '40', TRUE);
...
SELECT SUM(CASE WHEN gift ILIKE '%candle%' THEN 1 ELSE 0 END) sum FROM gifts;


--
\echo Get the AVERAGE value from all the gifts
SELECT AVG(value) as Average FROM gifts;
